...
앱에 코드 서명이 없어서 처음 열 때 Gatekeeper가 "확인되지 않은 개발자" 경고를 띄웁니다. 처음 한 번만 아래처럼 열면 그다음부터는 평소대로 실행됩니다.

1. 내려받은 `.zip`을 풀고 `CVDv2.app`을 `응용 프로그램` 폴더 등 원하는 곳으로 옮깁니다.
2. **우클릭(또는 Control-클릭) → 열기 → 열기**를 누릅니다.
