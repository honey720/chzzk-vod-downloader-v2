...
ICON_NAMES = ("pause", "resume", "retry", "folder", "delete")

_CACHE: dict[tuple[str, str, int, float], QPixmap] = {}


def action_pixmap(name: str, color: str, size: int, dpr: float = 1.0) -> QPixmap:
    """조작 아이콘 도형을 `size`px 정사각 픽스맵으로 돌려준다 — 캐시 공유.

    같은 (이름, 색, 크기, 배율)은 항상 같은 QPixmap 객체다. 배율(dpr)만큼
    크게 그린 뒤 `setDevicePixelRatio`로 논리 크기를 맞춰 HiDPI에서도
    선명하다.
    """
    ...
    _PAINTERS[name](painter, float(size), QColor(color))
    painter.end()
    _CACHE[key] = pixmap
    return pixmap
...
def _paint_pause(painter: QPainter, s: float, color: QColor) -> None:
    """일시정지 — 세로 막대 둘."""
    _fill(painter, color)
    radius = s * 0.08
...
def _paint_resume(painter: QPainter, s: float, color: QColor) -> None:
    """재개 — 오른쪽을 향한 삼각형(재생)."""
    _fill(painter, color)
    painter.drawPolygon(
        QPolygonF(
...
def _paint_retry(painter: QPainter, s: float, color: QColor) -> None:
...
def _paint_folder(painter: QPainter, s: float, color: QColor) -> None:
    """폴더 열기 — 탭이 달린 폴더 몸통."""
    _fill(painter, color)
    radius = s * 0.10
    path = QPainterPath()
...
def _paint_delete(painter: QPainter, s: float, color: QColor) -> None:
    """삭제 — 대각선 둘(✕)."""
...
class IconButton(QPushButton):
    ...
    def paintEvent(self, event) -> None:
        ...
        color = theme.current_tokens()[self.colorToken()]
        pixmap = action_pixmap(self._icon_name, color, size, self.devicePixelRatioF())
