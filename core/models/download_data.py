"""다운로드 한 건의 공유 데이터 — 엔진·서비스가 공유하는 홀더 (#61, #75).
...
기존 속성 이름은 위임 프로퍼티로 제공한다(엔진 호출부 호환). 엔진 내부 구현 변수
...
class DownloadData:
    """다운로드 엔진·서비스가 공유하는 데이터 홀더 (core 모델 위임 어댑터)."""
    ...
    @classmethod
    def from_content(cls, content: Content) -> "DownloadData":
        """이미 만들어진 Content 인스턴스를 소유하는 DownloadData를 만든다 (#75).

        DownloadService가 Content 하나로 다운로드를 시작할 때 사용한다.
        __init__은 필드 값으로 새 Content를 만들므로, 호출자가 가진 Content와
        데이터가 이원화되는 것을 막기 위해 같은 인스턴스를 그대로 연결한다.
        """
        data = cls(
            content.base_url,
            content.url,
