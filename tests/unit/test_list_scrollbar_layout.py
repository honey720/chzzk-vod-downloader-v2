...
오너가 정한 것:
  ① 세로 스크롤바는 항상 켜 둔다 — 생겼다 사라지며 뷰포트 폭(스크롤바 폭만큼)이 흔들려
     카드 폭이 변하던 것을 없앤다. **이 파일의 핵심 게이트.**
  ② 목록은 창 좌우 끝까지 가서 스크롤바가 창 우측 끝에 붙고, 창 쪽에 있던 좌우 여백
     (outerMargin)은 목록 안 카드 컨테이너의 패딩으로 옮긴다 — 카드의 보이는 왼쪽 위치는
     그대로다.
  ③ 하단 카운트 라벨은 자릿수 최대치(`999/999`)로 고정폭을 예약한다 — `0/0` 기준으로
     시작 때 고정된 창 최소폭을 `0/10`·`0/100`이 넘어서 좌우 스크롤 안전망이 뜨던 결함.
     창 최소폭을 내용 따라 다시 재지 않는다.

전부 폰트 무의존 기하 관계로 잰다. 고장 주입: 정책을 AsNeeded로 되돌리면 ①이, 컨테이너
패딩을 0으로 되돌리면 ②가, 라벨 고정폭을 빼면 ③이 실패한다.
"""
